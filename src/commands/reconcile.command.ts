 ...
 *
 * File: src/commands/reconcile.command.ts
 * Purpose: Command handler for @reconcile. Delegates to the shared AI artifact pipeline or performs local workflow actions.
 *
 * Usage rules:
...
export async function runReconcileCommand(args: CommandArgs): Promise<void> {
  const result = await runAiArtifactCommand(args, 'reconcile');
