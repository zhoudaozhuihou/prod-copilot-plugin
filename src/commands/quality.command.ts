 ...
 *
 * File: src/commands/quality.command.ts
 * Purpose: Command handler for @quality. Delegates to the shared AI artifact pipeline or performs local workflow actions.
 *
 * Usage rules:
...
export async function runQualityCommand(args: CommandArgs): Promise<void> {
  const result = await runAiArtifactCommand(args, 'quality');
