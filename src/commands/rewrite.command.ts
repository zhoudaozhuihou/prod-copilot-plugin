 ...
 *
 * File: src/commands/rewrite.command.ts
 * Purpose: Command handler for @rewrite. Delegates to the shared AI artifact pipeline or performs local workflow actions.
 *
 * Usage rules:
...
export async function runRewriteCommand(args: CommandArgs): Promise<CommandResult> {
  const result = await runAiArtifactCommand(args, 'rewrite');
