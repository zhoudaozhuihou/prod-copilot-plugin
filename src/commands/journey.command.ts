 ...
 *
 * File: src/commands/journey.command.ts
 * Purpose: Command handler for @journey. Delegates to the shared AI artifact pipeline or performs local workflow actions.
 *
 * Usage rules:
...
export async function runJourneyCommand(args: CommandArgs): Promise<CommandResult> {
  const result = await runAiArtifactCommand(args, 'journey');
