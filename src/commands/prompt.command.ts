 ...
 *
 * File: src/commands/prompt.command.ts
 * Purpose: Command handler for @prompt. Delegates to the shared AI artifact pipeline or performs local workflow actions.
 *
 * Usage rules:
...
export async function runPromptCommand(args: CommandArgs): Promise<CommandResult> {
  const result = await runAiArtifactCommand(args, 'prompt');
