 ...
 *
 * File: src/commands/datacontract.command.ts
 * Purpose: Command handler for @datacontract. Delegates to the shared AI artifact pipeline or performs local workflow actions.
 *
 * Usage rules:
...
export async function runDatacontractCommand(args: CommandArgs): Promise<void> {
  const result = await runAiArtifactCommand(args, 'datacontract');
