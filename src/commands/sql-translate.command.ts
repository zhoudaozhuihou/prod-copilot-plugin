 ...
 *
 * File: src/commands/sql-translate.command.ts
 * Purpose: Command handler for @sql-translate. Delegates to the shared AI artifact pipeline or performs local workflow actions.
 *
 * Usage rules:
...
export async function runSqlTranslateCommand(args: CommandArgs): Promise<void> {
  const result = await runAiArtifactCommand(args, 'sql-translate');
