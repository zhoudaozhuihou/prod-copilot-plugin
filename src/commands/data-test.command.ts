 ...
 *
 * File: src/commands/data-test.command.ts
 * Purpose: Command handler for @data-test. Delegates to the shared AI artifact pipeline or performs local workflow actions.
 *
 * Usage rules:
...
export async function runDataTestCommand(args: CommandArgs): Promise<void> {
  const result = await runAiArtifactCommand(args, 'data-test');
