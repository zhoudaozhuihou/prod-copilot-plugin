 ...
 *
 * File: src/commands/scheduler.command.ts
 * Purpose: Command handler for @scheduler. Delegates to the shared AI artifact pipeline or performs local workflow actions.
 *
 * Usage rules:
...
export async function runSchedulerCommand(args: CommandArgs): Promise<void> {
  const result = await runAiArtifactCommand(args, 'scheduler');
