 ...
 *
 * File: src/commands/python.command.ts
 * Purpose: Command handler for @python. Delegates to the shared AI artifact pipeline or performs local workflow actions.
 *
 * Usage rules:
...
export async function runPythonCommand(args: CommandArgs): Promise<void> {
  const result = await runAiArtifactCommand(args, 'python');
