 ...
 *
 * File: src/commands/feature.command.ts
 * Purpose: Command handler for @feature. Delegates to the shared AI artifact pipeline or performs local workflow actions.
 *
 * Usage rules:
...
export async function runFeatureCommand(args: CommandArgs): Promise<CommandResult> {
  const result = await runAiArtifactCommand(args, 'feature');
