 ...
 *
 * File: src/commands/dbschema.command.ts
 * Purpose: Command handler for @dbschema. Delegates to the shared AI artifact pipeline or performs local workflow actions.
 *
 * Usage rules:
...
export async function runDbschemaCommand(args: CommandArgs): Promise<void> {
  const result = await runAiArtifactCommand(args, 'dbschema');
