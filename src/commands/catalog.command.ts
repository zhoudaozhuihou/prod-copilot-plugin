 ...
 *
 * File: src/commands/catalog.command.ts
 * Purpose: Command handler for @catalog. Delegates to the shared AI artifact pipeline or performs local workflow actions.
 *
 * Usage rules:
...
export async function runCatalogCommand(args: CommandArgs): Promise<void> {
  const result = await runAiArtifactCommand(args, 'catalog');
