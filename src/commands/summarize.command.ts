 ...
 *
 * File: src/commands/summarize.command.ts
 * Purpose: Command handler for @summarize. Delegates to the shared AI artifact pipeline or performs local workflow actions.
 *
 * Usage rules:
...
export async function runSummarizeCommand(args: CommandArgs): Promise<CommandResult> {
  const result = await runAiArtifactCommand(args, 'summarize');
