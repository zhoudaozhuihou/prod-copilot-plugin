 ...
 *
 * File: src/commands/release.command.ts
 * Purpose: Command handler for @release. Delegates to the shared AI artifact pipeline or performs local workflow actions.
 *
 * Usage rules:
...
export async function runReleaseCommand(args: CommandArgs): Promise<CommandResult> {
  const result = await runAiArtifactCommand(args, 'release');
