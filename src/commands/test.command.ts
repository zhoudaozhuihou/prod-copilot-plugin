 ...
 *
 * File: src/commands/test.command.ts
 * Purpose: Command handler for @test. Delegates to the shared AI artifact pipeline or performs local workflow actions.
 *
 * Usage rules:
...
export async function runTestCommand(args: CommandArgs): Promise<CommandResult> {
  const result = await runAiArtifactCommand(args, 'test');
