 ...
 *
 * File: src/commands/cost.command.ts
 * Purpose: Command handler for @cost. Delegates to the shared AI artifact pipeline or performs local workflow actions.
 *
 * Usage rules:
...
export async function runCostCommand(args: CommandArgs): Promise<void> {
  const result = await runAiArtifactCommand(args, 'cost');
