 ...
 *
 * File: src/commands/compress.command.ts
 * Purpose: Command handler for @compress. Delegates to the shared AI artifact pipeline or performs local workflow actions.
 *
 * Usage rules:
...
export async function runCompressCommand(args: CommandArgs): Promise<CommandResult> {
  const result = await runAiArtifactCommand(args, 'compress');
