 ...
 *
 * File: src/commands/checklist.command.ts
 * Purpose: Command handler for @checklist. Delegates to the shared AI artifact pipeline or performs local workflow actions.
 *
 * Usage rules:
...
export async function runChecklistCommand(args: CommandArgs): Promise<CommandResult> {
  const result = await runAiArtifactCommand(args, 'checklist');
