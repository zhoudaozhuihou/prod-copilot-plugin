 ...
 *
 * File: src/commands/policy-review.command.ts
 * Purpose: Command handler for @policy-review. Delegates to the shared AI artifact pipeline or performs local workflow actions.
 *
 * Usage rules:
...
export async function runPolicyReviewCommand(args: CommandArgs): Promise<CommandResult> {
  const result = await runAiArtifactCommand(args, 'policy-review');
