 ...
 *
 * File: src/commands/doc-review.command.ts
 * Purpose: Command handler for @doc-review. Delegates to the shared AI artifact pipeline or performs local workflow actions.
 *
 * Usage rules:
...
export async function runDocReviewCommand(args: CommandArgs): Promise<CommandResult> {
  const result = await runAiArtifactCommand(args, 'doc-review');
