 ...
 *
 * File: src/commands/frontend.command.ts
 * Purpose: Command handler for @frontend. Delegates to the shared AI artifact pipeline or performs local workflow actions.
 *
 * Usage rules:
...
export async function runFrontendCommand(args: CommandArgs): Promise<void> {
  const result = await runAiArtifactCommand(args, 'frontend');
