 ...
 *
 * File: src/commands/data-review.command.ts
 * Purpose: Command handler for @data-review. Delegates to the shared AI artifact pipeline or performs local workflow actions.
 *
 * Usage rules:
...
export async function runDataReviewCommand(args: CommandArgs): Promise<void> {
  const result = await runAiArtifactCommand(args, 'data-review');
