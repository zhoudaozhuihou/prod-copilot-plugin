 ...
 *
 * File: src/commands/runbook.command.ts
 * Purpose: Command handler for @runbook. Delegates to the shared AI artifact pipeline or performs local workflow actions.
 *
 * Usage rules:
...
export async function runRunbookCommand(args: CommandArgs): Promise<void> {
  const result = await runAiArtifactCommand(args, 'runbook');
