 ...
 *
 * File: src/commands/backend.command.ts
 * Purpose: Command handler for @backend. Delegates to the shared AI artifact pipeline or performs local workflow actions.
 *
 * Usage rules:
...
export async function runBackendCommand(args: CommandArgs): Promise<void> {
  const result = await runAiArtifactCommand(args, 'backend');
